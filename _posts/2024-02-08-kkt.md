...
We define, $$g(u,v)=\min_xL(x,u,v) \quad [eqn (2)]$$
...
2. Complementary slackness
    ...
    > Why is there no complementary slackness for $$v_jl_j$$?
		> Because it is a equality constraint. At optimal $$x$$ it will always be $$0$$.
...
$$f(x^{*})= g(u^{*}, v^{*})$$
$$= \min_{x\quad}f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)$$
...
- Step 2: $$= \min_{x}\quad f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)$$
...
- $\leq$ in step 3
	...
	- $$ \min_{x\quad}f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)\leq f(x^*)+\sum\limits_{i=1}^mu^{*}_ih_i(x^*)+\sum\limits_{j=1}^rv^{*}_jl_j(x^*)$$
