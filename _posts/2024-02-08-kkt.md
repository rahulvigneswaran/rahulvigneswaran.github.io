...
$$\max_{u,v}\;\min_{x\quad}f(x)+\sum\limits_{i=1}^mu_ih_i(x)+\sum\limits_{j=1}^rv_jl_j(x)$$
...
## Necessity  :  $$\text{Optimality} \implies \text{KKT}$$
...
- Step 3: $$\leq f(x^*)+\sum\limits_{i=1}^mu^{*}_ih_i(x^*)+\sum\limits_{j=1}^rv^{*}_jl_j(x^*)$$
	- Lets understand how we went from step 2 to step 3. $$x^*$$ is a solution for the primal but for $$g$$ it is just some solution. There is no guarantee that it is optimal. Therefore it must be greater than equal to the actual optimal solution of $$f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)$$.
- Step 4:$$\leq f(x^*)$$
...
- $$\leq$$ in step 3
	...
	- $$ \min_{x}\;f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)\leq f(x^*)+\sum\limits_{i=1}^mu^{*}_ih_i(x^*)+\sum\limits_{j=1}^rv^{*}_jl_j(x^*)$$
  	...
  	- We already saw that $$x^*$$ is actually not an optimal solution for the LHS (that is $$g(x)$$) but is just some solution. So for the inequality to become an equality we have to somehow enforce that $x^*$ is also an optimal solution for $$g(x)$$ too.
		- Lets focus only on the LHS : $$\min_{x}\;f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)$$
			- Note that from eqn (1), $$L(x,u^{*},v^{*})=f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)$$ . So the LHS can also be written as $$\min_xL(x,u^{*},v^{*})$$.
...
---
## Sufficieny :  $$\text{Optimality} \impliedby \text{KKT}$$
...
Lets go through the above now step by step,
- Step 1: $$g(u^{*}, v^{*}) = \min_{x\quad}f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)$$
	- We defined this in eqn (2)
- Step 2: $$\min_{x\quad}f(x)+\sum\limits_{i=1}^mu^{*}_ih_i(x)+\sum\limits_{j=1}^rv^{*}_jl_j(x)=f(x^{*})+\sum\limits_{i=1}^mu^{*}_ih_i(x^{*})+\sum\limits_{j=1}^rv^{*}_jl_j(x^{*})$$ 
...
## Acknowledgements
I heavily borrowed from [Ryan Tibshrani](https://www.youtube.com/watch?v=upxERZ1dyCE)'s video lecture and notes for the same. There are other lectures of Ryan explaining KKT conditions but I personally felt this one was much more easier to understand.
